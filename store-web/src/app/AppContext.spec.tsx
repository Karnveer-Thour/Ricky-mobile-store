...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button
        data-testid="add-btn"
        onClick={() => addToCart(1, 101, "Matte Black", 2)}
      >
        Add Product 1
      </button>
      <button
        data-testid="add-another-btn"
        onClick={() => addToCart(1, 101, "Matte Black", 1)}
      >
        Add Product 1 again
      </button>
      ...
      <button
        ...
        onClick={() => setTrackedOrderId("order-123")}
      >
        Track Order
      </button>
      <button
        data-testid="input-chat-btn"
        onClick={() => setChatInput("Hello support!")}
      >
        Set Chat Input
      </button>
...
describe("AppContext & AppProvider Storefront State Provider", () => {
  ...
  it("should add items to cart and open cart panel", () => {
    ...
    expect(screen.getByTestId("cart-items").textContent).toContain(
      "Prod:1-Qty:2",
    );
  });
  ...
  it("should update tracked order ID state", () => {
    ...
    expect(screen.getByTestId("tracked-order-id").textContent).toBe(
      "order-123",
    );
  });
  ...
  it("should send chat message and receive canned support response after delay", () => {
    ...
    expect(messages.textContent).toContain(
      "support: Hi Ricky! Welcome to Ricky Mobile Store support. How can I help you today?",
    );
    ...
    expect(messages.textContent).toContain(
      "support: Thanks for your message! Our team will get back to you shortly. Typical response time is under 5 minutes.",
    );

    vi.useRealTimers();
  });

  it("should manage authentication state, login, and logout", async () => {
    const AuthTestComponent = () => {
      ...
      return (
        <div>
          ...
          <button data-testid="close-modal-btn" onClick={closeAuthModal}>
            Close Modal
          </button>
          <button
            data-testid="login-btn"
            onClick={() => login("khanna.buyer@gmail.com", "password123")}
          >
            Login
          </button>
    ...
    act(() => {
      screen.getByTestId("open-modal-btn").click();
    });
