...
export default function OrderTrackingPage() {
  ...
  const [transitStatus, setTransitStatus] = useState<"normal" | "delayed">(
    "normal",
  );
  ...
  const destinationCoords = { lat: 30.709, lng: 76.222 };
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      const riderIcon = L.divIcon({
        className:
          "bg-[var(--color-ricky-accent-blue)] text-white w-8 h-8 rounded-full flex items-center justify-center border-2 border-white shadow-[var(--shadow-ricky-md)] animate-pulse",
        html: "🛵",
        iconSize: [32, 32],
        iconAnchor: [16, 16],
      });
      ...
      const destIcon = L.divIcon({
        className:
          "bg-[var(--color-ricky-accent-red)] text-white w-8 h-8 rounded-full flex items-center justify-center border-2 border-white shadow-[var(--shadow-ricky-md)]",
        html: "📍",
        iconSize: [32, 32],
        iconAnchor: [16, 16],
      });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (nextTick === 2 || nextTick === 3) {
        ...
        setDelayReason(
          "Rider stuck in heavy traffic near Grand Trunk Road (Est. 5-8 min delay) 🌧️",
        );
      } else {
        ...
        nextLat =
          riderCoords.lat + (destinationCoords.lat - riderCoords.lat) * 0.25;
        nextLng =
          riderCoords.lng + (destinationCoords.lng - riderCoords.lng) * 0.25;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-24 pb-16 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-4">
        <div>
          <span
            className="text-xs text-[#00cfff] font-bold"
                ...
                {idx < STEPS.length - 1 && (
                  <div
                    className={`absolute top-4 left-1/2 w-full h-[2px] -z-10 ${
                      idx < activeStep
                        ? "bg-[var(--color-ricky-accent-green-light)]"
                        : "bg-white/5"
                    }`}
                  />
                )}
      ...
      <div className="bg-[#0e0e1c] border border-white/5 rounded-3xl overflow-hidden relative shadow-[var(--shadow-ricky-md)]">
        <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
          ...
          >
            {gpsLost
              ? "Signal lost"
              : `Rider position updated ${lastUpdated}s ago`}
          </span>
        </div>
        ...
        <div className="p-6 bg-[var(--color-ricky-accent-green)]/10 border-2 border-[var(--color-ricky-accent-green)] rounded-3xl space-y-4">
          <div className="flex items-center gap-2 text-[var(--color-ricky-accent-green-light)]">
            ...
            <h3 className="font-bold text-sm">
              Courier Has Arrived at Doorstep!
            </h3>
          </div>
          <p className="text-xs text-gray-600 leading-relaxed">
            Please share the 4-digit Handover OTP with Harpreet to complete
            verification and receive your device.
          </p>
      ...
      {handoverSuccess && (
        <div className="p-6 bg-green-500/10 border-2 border-green-500/30 rounded-3xl text-center space-y-2">
          ...
          <p className="text-xs text-gray-600">
            Thank you for shopping at Ricky Mobile Store. Enjoy your new
            smartphone!
          </p>
        </div>
      )}
