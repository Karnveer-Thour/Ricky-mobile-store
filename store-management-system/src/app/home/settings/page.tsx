...
function Page() {
  ...
  return (
    <div className="w-full sm:px-8 space-y-8 overflow-hidden">
      ...
      {/* Profile Section */}
      <section
        className={`flex flex-col sm:flex-row items-center justify-between border-b pb-6 ${isDark ? "border-slate-800" : "border-slate-200"}`}
      >
        <div
          className={`w-28 h-28 rounded-full overflow-hidden border-2 ${isDark ? "border-slate-700" : "border-slate-300 shadow-sm"}`}
        >
          <Image
      ...
      <section
        className={`flex flex-col sm:flex-row justify-between items-center gap-4 border-t pt-6 ${isDark ? "border-slate-800" : "border-slate-200"}`}
      >
        <div className="flex flex-col sm:flex-row items-center gap-2">
          <span
            className={`font-semibold text-base ${isDark ? "text-slate-200" : "text-slate-800"}`}
          >
            Theme:
          </span>
          <span
