...
function Multiselect({ Heading, children ,isDark=false}: MultiselectProps) {
  ...
  return (
    <div className="relative w-full">
      ...
      <button
        ref={buttonRef}
        className={`w-full h-full p-3 border  rounded-md shadow-sm text-sm  flex justify-between items-center cursor-pointer hover:ring-blue-500 hover:ring-2 hover:font-bold hover:text-blue-500` + (isDark ? " bg-gray-500 text-white border-white hover:ring-blue-400" : "border-gray-300 bg-white")}
      ...
      {selectorOpened && (
        <div
          ref={menuRef}
          className={"absolute w-full h-auto p-4 border mt-1 rounded-lg shadow-lg z-50" + (isDark ? " bg-gray-500 text-white border-white hover:ring-blue-400" : "border-gray-300 bg-white")}
        >
          {children}
        </div>
      )}
