...
export default function GlobalConfirmModal() {
  ...
  const getVariantStyles = () => {
    switch (variant) {
      case "danger":
        return {
          icon: <AlertOctagon size={28} className="text-rose-400" />,
      ...
      case "warning":
        return {
          ...
          confirmBtn:
            "bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-400 hover:to-yellow-500 text-slate-950 shadow-amber-950/50 font-bold",
          accentBorder: "border-amber-500/30",
        };
      default:
        return {
          icon: <Info size={28} className="text-cyan-400" />,
          badgeBg: "bg-cyan-500/15 border-cyan-500/30",
          confirmBtn:
            "bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 shadow-cyan-950/50 font-bold",
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className={`relative w-full max-w-md rounded-3xl overflow-hidden shadow-2xl border bg-slate-900/95 backdrop-blur-2xl border-slate-700/80 shadow-black/80 p-6 flex flex-col items-center text-center space-y-4`}
        >
          {/* Top Close Button */}
          {!loading && (
            <button
          ...
          {/* Icon Badge */}
          <div
            className={`p-3.5 rounded-2xl border ${currentVariant.badgeBg} shadow-xl`}
          >
            {currentVariant.icon}
          </div>
          ...
          <div className="w-full flex items-center justify-center gap-3 pt-2">
            <button
              ...
              disabled={loading}
              onClick={() => executeCancelAction(dispatch)}
              className="flex-1 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold border border-slate-700 transition-colors disabled:opacity-50 cursor-pointer"
            >
              {cancelText}
            </button>
