...
export const cityService = {
  ...
  async updateCity(
    ...
    try {
      ...
      if (cityData.isAccepting !== undefined) payload.isAccepting = cityData.isAccepting;

      const response = await fetch(`${API_BASE_URL}/accepted-cities/update/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
  ...
  async deleteCity(id: string | number): Promise<{ ok: boolean; message?: string }> {
    try {
